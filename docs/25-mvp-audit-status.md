...
- DB-backed integration tests for hosted-staging fetch authorization, expired objects, and full ingest/archive/verify flow are still missing; current coverage is unit-heavy.
...
1. Add DB-backed integration/smoke coverage for hosted-staging fetch authorization, expired/missing staging objects, and the local ingest/archive/verify path.
2. Implement restore-drill executor semantics now that hosted-staging archive placement has a supported data-plane source.
