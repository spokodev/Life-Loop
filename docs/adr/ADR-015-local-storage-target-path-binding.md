...
- when the agent has a device credential, it may compare local binding ids/providers with credential-scoped control-plane storage target metadata and report missing, extra, or mismatched bindings without uploading `rootPath`.
